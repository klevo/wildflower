...
class CommentsListHelper extends AppHelper {
    ...
    function __construct() {
       parent::__construct();
       $this->_emptyMessage = __('No comments yet', true) . '.' ;
    }
    ...
    function create($comments, $settings = array(), $spamList = false) {
    	...
    	foreach ($comments as $comment) {
	        ...
	        $listHtml .= $author;
	        $listHtml .= ' ' . sprintf(__('responded $s to', true),  $this->Time->timeAgoInWords($comment['Comment']['created'])) . ' '
	        ...
	        $spamLink = '';
	        if ($spamList) {
	            $spamLink = $this->Html->link(__('Not a spam', true), '#NotSpam', array('class' => 'unspam-comment'));
	        } else {
	            $spamLink = $this->Html->link(__('Mark as spam', true), '#Spam', array('class' => 'spam-comment'));
	        }
	        
	        $listHtml .= '<li>' . $spamLink . '</li>';
	        $listHtml .= '<li>' . $this->Html->link(__('Delete', true), '#Delete', array('class' => 'delete-comment')) . '</li>';
	        $listHtml .= '</ul>';
